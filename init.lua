...
HYPER_HYPER = {"ctrl", "cmd", "shift"} -- this should be system wide stuff
HYPER_LOCAL = {"cmd", "alt"} -- macros for local app
HYPER_NOPE = {"ctrl", "cmd", "alt"} -- only for noremap hacks
HYPER_WINDOW = {"cmd", "shift"} -- to control window manager (check Rectangle)
HYPER_RESERVED = {"ctrl", "shift"} -- reserved for in-app configurations
NONE = {} -- simple key, without modifier

-- Karabiner hacked modifiers ~/.config/karabiner/assets/complex_modifications
KARABINER = {
    left_shift = "s",
    left_control = "c",
    left_command = "h",
...
HK.bind(NONE, "F4",
        function() bring_or_hide_window(get_winger_window("Chat")) end)
HK.bind({"shift"}, "F4",
        function() bring_or_hide_window(get_winger_window("Chat", true)) end)
HK.bind(HYPER_WINDOW, "space", function() align_windows() end)
...
local function moveTabRight(steps)
    ...
    hs.eventtap.keyStroke(HYPER, "1")
end
local function duplicateTab()
    input_pause()
    hs.eventtap.keyStroke(HYPER, "l")
    -- hs.eventtap.keyStroke({}, "right")
    -- hs.eventtap.keyStrokes("#dup")
    hs.eventtap.keyStroke({"alt", "shift"}, "return")
end
tabqueueModal:bind({"control"}, "W",
                   function() hs.eventtap.keyStroke(HYPER, "w") end)
...
wf_notification = WF.new(false):setAppFilter("CoreServicesUIAgent",
...
wf_notification:subscribe({WF.windowCreated, WF.windowFocused}, function(win)
    if win:title() == "" then
        ...
        local buttons = ui:childrenWithRole("AXButton")
        for _, button in ipairs(buttons) do
            if button:attributeValue("AXTitle"):match("Keep.*Hammerspoon") then
...
wf_transmission:subscribe(WF.windowFocused,
                          function() transmissionModal:enter() end)
wf_transmission:subscribe(WF.windowUnfocused,
                          function() transmissionModal:exit() end)
...
HK.bind(HYPER_WINDOW, "delete",
        function() spoon.Split:deleteCurrentSplit(true) end)
...
function music(action)
    if action == "prev" then
        ...
        -- pianobar_control("like")
    elseif action == "next" then
        spotify_control("next")
        -- pianobar_control("next_song")
    elseif action == "next-playlist" then
        spotify_control("next-playlist")
    elseif action == "next_station" then
        spotify_control("next")
        -- pianobar_control("next_station")
    elseif action == "play" then
        -- pianobar_control("play")
        -- hs.spotify.play()
        spotify_control("play")
    elseif action == "playpause" then
        -- pianobar_control("playpause")
        -- hs.spotify.playpause()
        spotify_control("play-pause")
    elseif action == "stop" then
        -- pianobar_control("close")
        -- hs.spotify.pause()
        spotify_control("pause")
    elseif action == "volumen_up" then
        -- pianobar_control("volumen +2")
        -- hs.spotify.setVolume(hs.spotify.getVolume() + 5)
        spotify_control("volume -- +5")
    elseif action == "volumen_down" then
        -- pianobar_control("volumen -5")
        -- hs.spotify.setVolume(hs.spotify.getVolume() - 11)
        spotify_control("volume -- -11")
    else
        hs.alert("Action not implemented: " .. action)
    end
end

function paste_text()
    types = hs.pasteboard.typesAvailable()
    ...
    elseif types["image"] then
        ...
        hs.eventtap.keyStroke(HYPER, "v")
        -- hs.eventtap.keyStrokes("no conversion available")
    end
    inspect(types)
end
...
function setVolume(volume)
    ...
    new_volume = device:volume() + volume
    if new_volume < 0 then
        new_volume = 0
    elseif new_volume > 200 then
        new_volume = 100
    end
...
HK.bind({"shift"}, "F13", function() todoWindow({set = true}) end) -- actually Escape key after karabiner
